    ...
    update(game) {
        this.droplets.forEach(droplet => {
            ...
            for (let fire of game.fires) {
                if (fire.intensity > 0 && physics.checkWaterFireCollision(droplet, fire)) {
                    ...
                    // 创建水花粒子
                    particles.createSplash(droplet.x, droplet.y);
            ...
            for (let building of game.buildings) {
                if (building.health > 0 && physics.checkWaterBuildingCollision(droplet, building)) {
                    droplet.active = false;
